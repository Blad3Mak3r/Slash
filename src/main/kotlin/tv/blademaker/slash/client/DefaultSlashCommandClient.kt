...
class DefaultSlashCommandClient internal constructor(
    ...
    private val contextCreator: ContextCreator,
    ...
    override val registry = discoveryResult.also {
    ...
    private suspend fun createSlashCommandContext(
        handler: SlashCommandHandler,
        event: SlashCommandInteractionEvent
    ): SlashCommandContext {
        return when (handler.target) {
            InteractionTarget.GUILD -> contextCreator.createGuildContext(event, handler.function)
            InteractionTarget.DM -> contextCreator.createContext(event, handler.function)
            InteractionTarget.ALL -> when (event.isFromGuild) {
                true -> contextCreator.createGuildContext(event, handler.function)
                false -> contextCreator.createContext(event, handler.function)
            }
        }
    }
    ...
    private suspend fun onModalInteractionEvent(event: ModalInteractionEvent) {
        ...
        val ctx = ModalContext(event, handler.first, handler.second.function)
    ...
    private suspend fun onButtonInteractionEvent(event: ButtonInteractionEvent) {
        ...
        val ctx = ButtonContext(event, handler.first, handler.second.function)
    ...
    private suspend fun onMessageContextInteractionEvent(event: MessageContextInteractionEvent) {
        ...
        val ctx = MessageCommandContext(event)
    ...
    private suspend fun onUserContextInteractionEvent(event: UserContextInteractionEvent) {
        ...
        val ctx = UserCommandContext(event)
